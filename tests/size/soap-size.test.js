...
import { createLogger, toMB, byteSize } from "../../src/utils/index.js";
...
export default function (data) {
  ...
  const response = http.post(config.get("url"), body, {
    ...
    tags: { name: "DET-WS-SIZE", target_mb: String(targetMB), actual_mb: actualMB },
  });

  const checkResult = runSoapChecks(response, { name: "Size Test", size: targetMB });

  if (checkResult) {
    metrics.successRate.add(1);
    metrics.maxSizeReached.add(targetMB);
    logger.info("Size test passed", {
      targetMB, actualMB, status: response.status, duration: response.timings.duration,
    });
    if (__ITER === SIZE_SEQUENCE.length - 1) {
      logger.info("Maximum size reached without failure", {
        maxTestedMB: targetMB, recommendation: "Increase MAX_MB to continue testing",
      });
    }
  } else {
    ...
    logger.error("Size test FAILED - Breaking point found", {
      targetMB, actualMB, status: response.status,
...
export function teardown() {
  logger.info("Size test completed", {
    totalSizesTested: SIZE_SEQUENCE.length, sequence: SIZE_SEQUENCE,
  });
}
